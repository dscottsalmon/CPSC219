...
public class PredatorTest extends FormatTester{
	...
	public void test_Constructor_huntStyle_invalid(){
		...
		assertEquals("Created prey (f,25,camouflage) testing huntStyle", "stalk", c.getHuntStyle());
		assertEquals("Created prey with valid 25 health.", 25, c.getHealth());
		assertEquals("Created prey (f,25,camouflage) testing type", 'f', c.getType());

		c = new Predator('f',25,"nothing");
		assertEquals("Created prey (f,25,nothing) testing huntStyle", "stalk", c.getHuntStyle());
		assertEquals("Created prey with valid 25 health.", 25, c.getHealth());
		assertEquals("Created prey (f,25,nothing) testing type", 'f', c.getType());

		c = new Predator('f',25,"");
		assertEquals("Created prey (f,25,'') testing huntStyle", "stalk", c.getHuntStyle());
		assertEquals("Created prey with valid 25 health.", 25, c.getHealth());
...
// Testing setter and getters
	...
	public void test_getRelativeHealth_stalk(){
		...
		assertEquals("Stalk huntStyle and health 10.", 15.0, p.getRelativeHealth(),0.000001);
	}
	
	@Test
	public void test_getRelativeHealth_stalk2(){
		testInterface();
		Predator p = new Predator('m',123,"stalk");
		assertEquals("Stalk huntStyle and health 123.", 184.5, p.getRelativeHealth(),0.000001);
	}
	...
	public void test_getRelativeHealth_ambush(){
		...
		assertEquals("Ambush huntStyle and health 10.", 7.5, p.getRelativeHealth(),0.000001);
	}
	
	@Test
	public void test_getRelativeHealth_ambush2(){
		testInterface();
		Predator p = new Predator('m',43,"ambush");
		assertEquals("Ambush huntStyle and health 43.", 32.25, p.getRelativeHealth(),0.000001);
	}
	...
	public void test_toString() {
		...
		Predator c = new Predator('b', 51, "ambush");

		assertEquals("[Predator] Type: b Health: 38.25 Hunt style: ambush", c.toString());
	}
	...
	public void test_toString2() {
		testInterface();
		assertTrue("Should override toString and it should invoke parent toString (not getter methods in parent).", toStrInvokesParentToStr());
		Predator c = new Predator('f', 5, "stalk");

		assertEquals("[Predator] Type: f Health: 7.5 Hunt style: stalk", c.toString());
	}
	...
	public void test_getStatusInParent_criticalAndTenuous(){
		testInterface();
		Predator c = new Predator('a', 1, "stalk");
		String actualStatus = c.getStatus();
		assertEquals("health of 1, stalk", "critical", actualStatus);

		c = new Predator('a', 50, "ambush");
		actualStatus = c.getStatus();
		assertEquals("health of 50, ambush", "tenuous", actualStatus);
	}
	@Test
	public void test_getStatusInParent_goodAndExcellent(){
		testInterface();
		Predator c = new Predator('a', 40, "stalk");
		String actualStatus = c.getStatus();
		assertEquals("health of 50, stalk", "good", actualStatus);

		c = new Predator('a', 100, "ambush");
		actualStatus = c.getStatus();
		assertEquals("health of 100, ambush", "excellent", actualStatus);
	}
}
